use std::{collections::{BinaryHeap, HashMap}, iter::repeat};

pub struct Solution {}

impl Solution {
    pub fn frequency_sort(s: String) -> String {
        let mut frequent_map: HashMap<u8, usize> = HashMap::new();
        let mut res = "".to_string();

        for c in s.into_bytes().into_iter() {
            frequent_map.entry(c).and_modify(|counter| *counter += 1).or_insert(1);
        }

        let mut frequent_heap: BinaryHeap<(usize, u8)> = BinaryHeap::new();
