...
fn dp_mc(coins: &[u32], amount: u32, min_coins: &mut[u32], used_coins: &mut [u32]) -> u32 {
    //动态收集找零值从1到amount的最优解
    for denm in 1..=amount {
        //最差的情况是全部用1找零
        let mut min_coin_num = denm;
        let mut cur_used_coin = 1;
        ...
        used_coins[denm as usize] = cur_used_coin;
    }
    return min_coins[amount as usize];
}

fn print_coins(used_coins: &[u32], mut amount: u32) {
    while amount > 0 {
        let cur = used_coins[amount as usize];
        println!("{}", cur);
        amount -= cur;
    }
}
...
fn main() {
    ...
    let mut used_coins = [0; 82];

    let coin_num = dp_mc(&coins, amount, &mut min_coins, &mut used_coins);
